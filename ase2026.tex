...
Despite this progress, existing build-failure RCA pipelines still largely rely on a ``short window + local features'' paradigm: they either depend on handcrafted heuristics that are brittle across heterogeneous logs and evolving software versions, or directly truncate long logs before reasoning, which breaks cross-stage causal chains. In addition, many systems emphasize error-type prediction or loosely constrained recommendations, without explicitly modeling applicability conditions in historical repair knowledge. These gaps motivate our framework design: preserving long-range evidence in Step~1 and enforcing condition-aware, traceable rule-tree reasoning in Step~2.

\begin{figure*}[!tb]
    \centering
    \includegraphics[width=\textwidth]{pic/mgsc-overview.png}
    \caption{Overall workflow of Unravel Step 1: Long-Range Evidence Extraction from Build Logs}
    \Description{A three-step pipeline diagram: Step 1 performs coarse-grained filtering with error anchor localization, Step 2 performs entity-based context backtracking, and Step 3 performs fine-grained semantic summarization.}
    \label{fig:mgsc}
\end{figure*}
...
\begin{figure*}[!t]
    ...
    \caption{Unravel Step 2: Rule-Tree Reasoning for Traceable Root-Cause Diagnosis}
...
\item \textbf{RQ1 (Extraction Effectiveness):} How well does Step 1 preserve diagnosis-critical evidence compared with representative extraction baselines?
\item \textbf{RQ2 (Extraction Ablation):} How much does each Step-1 module contribute to final extraction quality?
...
\begin{sloppypar}
Our experiments are conducted on real build-failure logs collected from a production OBS environment for RISC-V package migration. We use a shared dataset of 127 build-failure logs (149{,}219 lines in total) for both steps of Unravel, but evaluate them with separate protocols: Step 1 is evaluated as an evidence-extraction task, while the full pipeline is evaluated as a root-cause-analysis task. For Step-1 evaluation, domain experts annotate 4{,}582 repair-relevant lines as ground truth. For full-pipeline evaluation, each sample in this same 127-case dataset is evaluated as one failure case. The five RCA categories are architecture compatibility, version compatibility, dependency configuration, compilation configuration, and other errors. The Step-1 fine-tuning dataset (2,925 examples) is collected and split independently, with no overlap with this 127-case evaluation dataset.
...
For Unravel and comparable baselines, we use DeepSeekV3 and GPT-OSS as backbones. Standalone reasoning models are evaluated end-to-end using Step-1 structured extraction outputs as inputs. The Step-1 extractor is fine-tuned from Qwen3-4B-Instruct with LoRA-based parameter-efficient tuning (Section~\ref{sec:ceel}). The rule-tree knowledge base is built offline from historical OBS repair commits using the pipeline in Section~\ref{sec:hrl}.
...
\subsection{RQ1: Effectiveness of Unravel Step 1 for Evidence Extraction}
...
Unravel Step 1 (ours) & 0.308 & 0.671 & \textbf{0.423} \\
\hline
\end{tabular}
\end{table}

Table~\ref{tab:mgsc-results} shows that Unravel Step 1 achieves the best overall balance among the compared methods: it obtains the highest F1 score (0.423) while maintaining high recall (0.671). Compared with LogSage, Step 1 improves recall by approximately 15.5\% and F1 by about 3.7\%, indicating that it preserves more key repair evidence without introducing excessive noise. TemplateRAG reaches the highest recall (0.764) but suffers from extremely low precision (0.047), suggesting heavy over-retention of irrelevant content. In contrast, Step 1 keeps precision and recall in a more practical range for downstream diagnosis. In terms of token efficiency, Step 1's average output lengths for three log groups are 3442/4234/5502 tokens, far smaller than TemplateRAG (38746/49883/55406), leaving substantially more context budget for the reasoning step. Runtime results also show that Step 1 provides about 2.3--3.6$\times$ acceleration over LogPrompt and TemplateRAG on ultra-long logs.

\subsection{RQ2: Contribution of Each Step-1 Component}

We perform an ablation study to assess the contribution of each Step-1 component.

\begin{table}[htbp]
\centering
\caption{Ablation results of Unravel Step 1}
\label{tab:mgsc-ablation}
\begin{tabular}{lccc}
\hline
\textbf{Configuration} & \textbf{Prec} & \textbf{Rec} & \textbf{F1} \\
\hline
Step 1 (full) & 0.308 & 0.671 & 0.423 \\
